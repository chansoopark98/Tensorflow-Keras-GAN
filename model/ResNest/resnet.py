...
def resnest_block(x, n_filter, stride_size=1, dilation=1, group_size=1, radix=1, block_width=64, avd=False,
    ...
    if avd:
        avd_layer = tf.keras.layers.AveragePooling2D(3, strides=stride_size, padding="same",
                                                     name="stage{0}_block{1}_avd".format(stage, index))
        stride_size = 1
        if avd_first:
            out = avd_layer(out)

    if 0 < radix:
        out = split_attention_block(out, group_width, 3, stride_size, dilation, group_size, radix, dropout_rate,
                                    expansion, prefix="stage{0}_block{1}".format(stage, index))
    else:
    ...
    out = tf.keras.layers.Activation(tf.keras.activations.relu,
                                     name="stage{0}_block{1}_shorcut_act".format(stage, index))(out)
    return out


def resnest_module(x, n_filter, n_block, stride_size=1, dilation=1, group_size=1, radix=1, block_width=64,
                   avg_down=True, avd=False, avd_first=False, dropout_rate=0., expansion=4, is_first=True, stage=1):
    downsample = None
    if stride_size != 1 or tf.keras.backend.int_shape(x)[-1] != (n_filter * expansion):
        if avg_down:
            if dilation == 1:
                downsample = tf.keras.layers.AveragePooling2D(stride_size, strides=stride_size, padding="same",
                                                              name="stage{0}_downsample_avgpool".format(stage))(x)
            else:
                downsample = tf.keras.layers.AveragePooling2D(1, strides=1, padding="same",
                                                              name="stage{0}_downsample_avgpool".format(stage))(x)
    ...
    if dilation == 1 or dilation == 2 or dilation == 4:
        out = resnest_block(x, n_filter, stride_size, 2 ** (dilation // 4), group_size, radix, block_width, avd,
                            avd_first, downsample, dropout_rate, expansion, is_first, stage=stage)
    else:
        raise ValueError("unknown dilation size '{0}'".format(dilation))

    for index in range(1, n_block):
        out = resnest_block(out, n_filter, 1, dilation, group_size, radix, block_width, avd, avd_first,
                            dropout_rate=dropout_rate, expansion=expansion, stage=stage, index=index + 1)
    return out


def ResNet(x, stack, n_class=1000, include_top=True, dilation=1, group_size=1, radix=1, block_width=64, stem_width=64,
           deep_stem=False, dilated=False, avg_down=False, avd=False, avd_first=False, dropout_rate=0., expansion=4):
    # Stem
    if deep_stem:
    ...
    # Stage 1
    out = resnest_module(out, 64, stack[0], 1, 1, group_size, radix, block_width, avg_down, avd, avd_first,
                         expansion=expansion, is_first=False, stage=1)
    # Stage 2
    out = resnest_module(out, 128, stack[1], 2, 1, group_size, radix, block_width, avg_down, avd, avd_first,
                         expansion=expansion, stage=2)
    ...
    # Stage 3
    out = resnest_module(out, 256, stack[2], stride_size[0], dilation[0], group_size, radix, block_width, avg_down, avd,
                         avd_first, dropout_rate, expansion, stage=3)
    # Stage 4
    out = resnest_module(out, 512, stack[3], stride_size[1], dilation[1], group_size, radix, block_width, avg_down, avd,
                         avd_first, dropout_rate, expansion, stage=4)
